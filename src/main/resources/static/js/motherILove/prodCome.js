...
var RCDOperation = (function ($) {
    ...
    return {
        ...
        query: function (pageNum, pageSize) {
            var url = "/prodCome/pagination";
            $.get(url, function (result) {
                queryApp.prodComes = result.data;
        ...
        getProds: function () {
            ...
            $.get("/prod/getList", null, function (data) {
                if (data.state) {
                    result = data.data;
                } else {
                    return null;
        ...
        getMembers: function () {
            var result = '';
            $.ajaxSettings.async = false;
            $.get("/member/getList?type=2", null, function (data) {
                result = data;
            });
            return result;
            $.ajaxSettings.async = true;
        ...
        change: function () {
            if (updateApp.batch && updateApp.prodName  && updateApp.count && updateApp.price && updateApp.comeTime) {
...
var queryApp = new Vue({
    ...
    data: {
        search: '',
        prodComes: '',
        batch: RCDOperation.getBatch(), // 节点类型（AJAX获取）
        total: '',
        pageNum: 1,
        pages: '',
        pageSize: 5,
    ...
    methods: {
        query: function (currentPage, pageSize) {
            var url = "/prodCome/pagination?currentPage=" + currentPage + "&pageSize=" + pageSize;
            var _self = this;
            $.get(url, function (result) {
        ...
        deleteById: function (id) {
            layer.confirm("确认要删除吗，删除后不能恢复", {title: "删除确认"}, function (index) {
                layer.close(index);
                $.get("/prodCome/delete", {id: id}, function (data) {
                    layer.alert(data, {
                            title: "删除操作",
                            btn: ['确定']
                        },
        ...
        openAddDialog: function () {
            ...
            updateApp.id = '';
            updateApp.num = ''; // 节点类型（AJAX获取）
            updateApp.batch = RCDOperation.getBatch(); // 节点类型（AJAX获取）
            updateApp.prods = RCDOperation.getProds();//地区
            updateApp.prodName = updateApp.prods[0]==null?'':updateApp.prods[0].id;//地区
            updateApp.members = RCDOperation.getMembers();//地区
            updateApp.memberName = updateApp.members[0]==null?'':updateApp.members[0].id;  // 信用代码
            updateApp.price = '';  // 信用代码
            updateApp.count = '';  // 信用代码
            // updateApp.comeTime = "2013-02-15 12:29";  // 信用代码
            updateApp.comeTime = RCDOperation.getDate();  // 信用代码
        ...
        save: function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    memberId: this.memberName,
                    price: this.price,
                    comeCount: this.count,
                    comeTime: this.comeTime,
                },
